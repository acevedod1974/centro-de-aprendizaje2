...
const EngranajCalculator: React.FC = () => {
  ...
  const applications = {
    general: { name: 'Uso General', serviceFactor: 1.0, description: 'Aplicaciones estándar' },
    pesado: { name: 'Servicio Pesado', serviceFactor: 1.5, description: 'Cargas altas continuas' },
    intermitente: { name: 'Servicio Intermitente', serviceFactor: 0.8, description: 'Operación ocasional' },
    precision: { name: 'Alta Precisión', serviceFactor: 1.2, description: 'Tolerancias estrechas' }
  };
  ...
  const allowableLoad = (materialData.allowableStress * faceWidth * module * lewisFormFactor) / appData.serviceFactor;

  // Velocidad tangencial (asumiendo 1000 RPM para el piñón)
  const assumedRPM = 1000;
  const tangentialVelocity = (Math.PI * pitchDiameterPinion * assumedRPM) / (60 * 1000); // m/s

  // Volumen y peso aproximado
  const volumePinion = (Math.PI * Math.pow(outsideDiameterPinion/2, 2) * faceWidth) / 1000000; // m³
  const volumeGear = (Math.PI * Math.pow(outsideDiameterGear/2, 2) * faceWidth) / 1000000; // m³
  const weightPinion = volumePinion * materialData.density; // kg
  const weightGear = volumeGear * materialData.density; // kg
  ...
  const getApplicationColor = (app: string) => {
    switch (app) {
      ...
      case 'pesado': return 'text-red-600';
      case 'intermitente': return 'text-green-600';
      case 'precision': return 'text-purple-600';
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden">
        <div className="bg-gradient-to-r from-purple-600 to-indigo-600 p-6 text-white">
          <div className="flex items-center space-x-3">
            <Cog size={28} />
            <div>
              <h2 className="text-2xl font-bold">Calculadora de Engranajes Cilíndricos</h2>
              <p className="opacity-90">Diseño y análisis de transmisiones por engranajes</p>
            </div>
          </div>
        </div>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white dark:bg-gray-700 rounded-lg p-6 border border-purple-200 dark:border-purple-700">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="space-y-3">
                    <h4 className="font-medium text-gray-900 dark:text-white">Piñón</h4>
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        <span>Diámetro Primitivo:</span>
                        <span className="font-bold">{pitchDiameterPinion.toFixed(2)} mm</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Diámetro Exterior:</span>
                        <span className="font-bold">{outsideDiameterPinion.toFixed(2)} mm</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Diámetro Interior:</span>
                        <span className="font-bold">{rootDiameterPinion.toFixed(2)} mm</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Peso Aproximado:</span>
                        <span className="font-bold">{weightPinion.toFixed(2)} kg</span>
                      </div>
                    </div>
                  </div>
                  
                  <div className="space-y-3">
                    <h4 className="font-medium text-gray-900 dark:text-white">Engranaje</h4>
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        <span>Diámetro Primitivo:</span>
                        <span className="font-bold">{pitchDiameterGear.toFixed(2)} mm</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Diámetro Exterior:</span>
                        <span className="font-bold">{outsideDiameterGear.toFixed(2)} mm</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Diámetro Interior:</span>
                        <span className="font-bold">{rootDiameterGear.toFixed(2)} mm</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Peso Aproximado:</span>
                        <span className="font-bold">{weightGear.toFixed(2)} kg</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-white dark:bg-gray-700 rounded-lg p-6 border border-blue-200 dark:border-blue-700">
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg text-center">
                    ...
                    <div className="text-sm text-gray-600 dark:text-gray-400">Relación de Transmisión</div>
                  </div>
                  
                  <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg text-center">
                    <div className="text-2xl font-bold text-green-600 mb-1">
                      {centerDistance.toFixed(2)}
                    </div>
                    <div className="text-sm text-gray-600 dark:text-gray-400">Distancia entre Centros (mm)</div>
                  </div>
                  ...
                  <div className="bg-orange-50 dark:bg-orange-900/20 p-4 rounded-lg text-center">
                    ...
                    <div className="text-sm text-gray-600 dark:text-gray-400">Paso Circular (mm)</div>
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-4">
                  <div className="grid md:grid-cols-2 gap-4">
                    <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg">
                      <div className="text-sm text-gray-600 dark:text-gray-400 mb-1">Carga Admisible</div>
                    ...
                    <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
                      <div className="text-sm text-gray-600 dark:text-gray-400 mb-1">Velocidad Tangencial</div>
                  ...
                  <div className="p-4 bg-gray-50 dark:bg-gray-600 rounded-lg">
                    <div className="text-sm text-gray-600 dark:text-gray-400 mb-2">Configuración Actual:</div>
                    <div className="flex flex-wrap gap-4 text-sm">
                      <span><strong>Material:</strong> {materialData.name}</span>
                      <span><strong>Aplicación:</strong> {appData.name}</span>
                      <span className={`font-medium ${getApplicationColor(application)}`}>
              ...
              <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
                <h3 className="text-lg font-semibold mb-4">Representación Esquemática</h3>
                
                <div className="relative mx-auto" style={{ width: '400px', height: '200px' }}>
                  {/* Pinion */}
                  <div 
                  ...
                  >
                    <span className="text-xs font-bold text-white">{teethPinion}T</span>
                  </div>
                  ...
                  >
                    <span className="text-xs font-bold text-white">{teethGear}T</span>
                  </div>
              ...
              <div className="bg-yellow-50 dark:bg-yellow-900/20 p-6 rounded-lg">
                ...
                <ul className="text-sm text-yellow-700 dark:text-yellow-300 space-y-1">
                  <li>• Verificar que el número mínimo de dientes evite interferencia</li>
                  <li>• Considerar lubricación adecuada para la velocidad calculada</li>
                  <li>• Evaluar la necesidad de tratamiento térmico según la aplicación</li>
                  <li>• Verificar tolerancias de fabricación y montaje</li>
                  <li>• Considerar factores dinámicos para altas velocidades</li>
                  <li>• Analizar deflexiones en ejes para anchos de cara grandes</li>
                </ul>
              </div>
            </div>
          </div>
