...
const MecanizadoSimulator: React.FC = () => {
  ...
  const materials = {
    ...
    inoxidable: { name: 'Acero Inoxidable', hardness: 1.3, machinability: 0.6 },
  ...
  useEffect(() => {
    ...
    if (isRunning) {
      interval = setInterval(() => {
        ...
        const machineData = machineTypes[machineType as keyof typeof machineTypes];
        
        // Calculate material removal rate
        const mrr = (spindleSpeed * feedRate * depthOfCut) / 1000000 * materialData.machinability * machineData.efficiency;
        setMaterialRemoved(prev => prev + mrr);
        
        // Calculate tool wear
        const wearRate = (spindleSpeed / 10000) * materialData.hardness / toolData.durability;
        setToolWear(prev => Math.min(prev + wearRate, 100));
        
      }, 100);
    }
    
    return () => clearInterval(interval);
  }, [isRunning, spindleSpeed, feedRate, depthOfCut, material, toolMaterial, machineType]);
  ...
  const getEfficiencyRating = () => {
    ...
    if (efficiency > 1.2) return { rating: 'Excelente', color: 'text-green-600' };
    if (efficiency > 0.8) return { rating: 'Buena', color: 'text-blue-600' };
    if (efficiency > 0.5) return { rating: 'Regular', color: 'text-yellow-600' };
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden">
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-6 text-white">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-2xl font-bold">Simulador de Mecanizado CNC</h2>
              <p className="opacity-90">Optimiza parámetros de corte para diferentes materiales</p>
            </div>
          </div>
        </div>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-8">
                <h3 className="text-lg font-semibold mb-6 text-center">Estado del Proceso</h3>
                
                {/* Machine Representation */}
                <div className="relative mx-auto mb-6" style={{ width: '300px', height: '200px' }}>
                  ...
                  <div className="absolute top-1/2 right-1/4 transform -translate-y-1/2">
                    <div className={`w-8 h-2 ${toolWear > 70 ? 'bg-red-500' : 'bg-yellow-500'} transition-colors`}></div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">Material Removido:</span>
                    <span className="font-medium ml-2">{materialRemoved.toFixed(2)} cm³</span>
                  </div>
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">Eficiencia:</span>
                    <span className={`font-medium ml-2 ${efficiency.color}`}>{efficiency.rating}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 p-6 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">Máquina:</span>
                    <span className="font-medium ml-2">{machineTypes[machineType as keyof typeof machineTypes].name}</span>
                  </div>
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">Material:</span>
                    <span className="font-medium ml-2">{materials[material as keyof typeof materials].name}</span>
                  </div>
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">Herramienta:</span>
                    <span className="font-medium ml-2">{toolMaterials[toolMaterial as keyof typeof toolMaterials].name}</span>
                  </div>
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">Velocidad de Corte:</span>
                    <span className="font-medium ml-2">{Math.round((spindleSpeed * Math.PI * 50) / 1000)} m/min</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-yellow-50 dark:bg-yellow-900/20 p-6 rounded-lg">
                ...
                <ul className="text-sm text-yellow-700 dark:text-yellow-300 space-y-1">
                  {toolWear > 80 && <li>• ⚠️ Cambiar herramienta - desgaste crítico</li>}
                  {spindleSpeed > 3000 && material === 'acero' && <li>• Reducir velocidad para acero</li>}
                  {feedRate < 50 && <li>• Aumentar avance para mayor productividad</li>}
